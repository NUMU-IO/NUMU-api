...
async def _enqueue_meta_purchase(db: AsyncSession, order: Any) -> None:
    """Enqueue a Purchase CAPI event for ``order``, gated on the store config.

    Source of truth for Purchase per plan §5.4:
    - event_id = order.id (deterministic, dedupes against browser fire)
    - event_time = order.paid_at
    - custom_data carries the line-item snapshot in Meta's expected shape
    """
    # Lazy import — keeps the webhook module light on cold start when
    # CAPI fan-out isn't configured for any store.
...
async def paymob_callback(
    ...
    if is_refunded:
        ...
        # Purchase per plan §5.4. event_id = order.id is stable and
        # dedupes against the browser-side fire (when present).
        try:
