"""Customer import routes — CSV upload, mapping review, execute.

Flow (mirrors order_import):
  POST /stores/{id}/customers/import/preview  (multipart file) → columns + mapping
  POST /stores/{id}/customers/import/execute  (file + confirmed mapping) → results
...
(if any) from ``store.settings["customer_import_mapping"]`` is overlayed so the
...
from src.application.services.customer_import_service import (
...
router = APIRouter(prefix="/{store_id}/customers/import")


@router.post(
    "/preview",
    summary="Preview a CSV customer import (parses headers + suggests mapping)",
    operation_id="preview_customer_import",
)
async def preview_customer_import(
    ...
    service = CustomerImportService(customer_repo, store_repo)
...
@router.post(
    "/execute",
    summary="Execute a CSV customer import with a confirmed mapping",
    operation_id="execute_customer_import",
)
async def execute_customer_import(
    store_id: UUID,
    store: Annotated[Store, Depends(verify_store_ownership)],
    customer_repo: Annotated[CustomerRepository, Depends(get_customer_repository)],
    store_repo: Annotated[StoreRepository, Depends(get_store_repository)],
    ...
    if save_mapping:
        ...
        try:
            ...
            logger.exception(
                "customer_import_save_mapping_failed store=%s", str(store.id)
            )
...
@router.get(
    "/target-fields",
    summary="List the target fields the customer importer understands",
    operation_id="list_customer_import_target_fields",
)
