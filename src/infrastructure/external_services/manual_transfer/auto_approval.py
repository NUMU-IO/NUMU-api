"""Pure rules engine for auto-approving manual-payment proofs.

Rail-agnostic: it never asks which method an intent is on, only for
already-resolved facts. Two names below are historical and mean
"the merchant's destination on whichever rail this is" — the IPA for
InstaPay, the wallet number for Vodafone Cash:

  * ``AutoApprovalConfig.require_ocr_ipa_match``
  * ``AutoApprovalFacts.merchant_ipa``

They keep their names because both are persisted — the flag as a key
in ``store.settings``, the ``ocr_ipa_mismatch`` reason tag in
``payment_proofs.auto_approval_block_reasons`` rows and in the hub's
review copy. Renaming would be a data migration for a cosmetic win.
...
from src.core.entities.instapay import ManualPaymentIntent, PaymentProof
...
class AutoApprovalFacts:
    ...
    # Phase C — the merchant's stored destination on this rail (IPA
    # for InstaPay, wallet number for Vodafone Cash), compared
    # against ``proof.ocr_extracted_ipa``. Null means we don't have
    # it in hand for some reason; the rule no-ops.
