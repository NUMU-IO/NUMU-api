...
# The wordmark is shipped as an inline base64 PNG so it renders identically
# in every email client (Gmail strips Google Fonts; system serif fallbacks
# look generic). Generated by `scripts/generate_wordmark.py`.
from src.infrastructure.external_services.resend.email_templates._assets.wordmark import (  # noqa: E402
    WORDMARK_HEIGHT,
    WORDMARK_WHITE_PNG_DATA_URL,
    WORDMARK_WIDTH,
)
...
def _logo_block(language: str = "ar") -> str:
    """Render the نُمو brand mark as an inline PNG image.

    Display height is fixed so the wordmark sits comfortably
    inside the email header regardless of the rendered PNG aspect ratio.
    """
    display_h = 150
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin:0 auto;">
      <tr>
        <td align="center" style="text-align:center;">
          <img src="{WORDMARK_WHITE_PNG_DATA_URL}"
               width="{display_w}" height="{display_h}"
               alt="نُمو"
               style="display:block;width:{display_w}px;height:{display_h}px;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic;">
        </td>
      </tr>
      <tr>
        <td align="center" style="padding-top:14px;text-align:center;">
          <div style="font-family:{FONT_LATIN};font-size:11px;line-height:1;letter-spacing:5px;color:#D4AF37;text-transform:uppercase;font-weight:600;">
            N&nbsp;U&nbsp;M&nbsp;U
          </div>
        </td>
      </tr>
...
def header(
    ...
    return f"""
    <!--[if mso]>
    <v:rect xmlns:v="urn:schemas-microsoft-com:vml" fill="true" stroke="false" style="mso-width-percent:1000;height:280px;">
      <v:fill type="solid" color="{HERO_BG}"/>
      <v:textbox inset="0,0,0,0">
    <![endif]-->
    <div class="header" style="{header_bg_style}padding:48px 28px 40px;text-align:center;">
      {_logo_block(language)}
      <h1 style="color:#ffffff;margin:24px 0 8px;font-size:24px;font-weight:700;font-family:{font_family};line-height:1.4;">{title}</h1>
      {sub_html}
      {badge_html}
      <div style="height:3px;background:linear-gradient(90deg,transparent 0%,{GOLD} 50%,transparent 100%);margin:30px -28px -40px;"></div>
    </div>
    <!--[if mso]>
      </v:textbox>
    </v:rect>
