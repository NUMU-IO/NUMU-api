...
class FawryWebhookService:
    ...
    async def handle_paid(
        ...
        # picks up missed events). Implementation lives in the shared
        # dispatcher so all payment methods ship one identical payload.
        try:
            from src.application.services.meta_capi_purchase_dispatcher import (
                enqueue_meta_capi_purchase,
            )

            await enqueue_meta_capi_purchase(self.db, order)
        except Exception:
            logger.warning(
