...
2. **Scan for trigger evidence** against the six trigger categories:
   ...
   - `ai_llm` — keywords/patterns: LLM, Codex, GPT, prompt, scoring, classification (when LLM-based), summary generation, hallucination.
...
3. **Dispatch all adversary agents in a single parallel batch** using the host agent's sub-agent / task-dispatch primitive (e.g., Codex's Agent tool). All calls go in the same tool-use message so they run concurrently.
