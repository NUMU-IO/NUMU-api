...
- **FR-007a**: The storefront-facing opt-in endpoint MUST require a valid checkout-session token. The handler MUST: (a) verify the session token resolves to a Redis-backed checkout session associated with the addressed store, (b) confirm the session's recorded phone (set at issue time) equals the canonicalized phone supplied in the request body, both compared as E.164, (c) confirm the session is not expired (≤30 min TTL), and (d) reject the request (HTTP 403) on any failure with `invalid_checkout_session` (missing/expired/wrong store) or `phone_mismatch_with_cart` (token valid but phone differs). This closes the abuse vector where anyone with a store slug could write opt-in rows for arbitrary phones. The merchant-facing opt-in creation endpoint (auth-required) is unaffected by this rule.
- **FR-007b**: The system MUST provide a storefront-facing `POST /storefront/{store_slug}/checkout-session` endpoint that issues a fresh checkout-session token bound to the caller's current cart (resolved via the existing `numu_cart_session` cookie) and the phone supplied in the body. The endpoint accepts `{ phone, locale }`, canonicalizes the phone to E.164, persists a Redis row `{ token, cart_session_id, store_id, phone, issued_at, expires_at }` with a 30-min TTL, and returns the opaque token. Storefront calls this endpoint at the checkout Contact step (the moment the customer enters their phone) and passes the returned token to subsequent anonymous storefront endpoints that require phone-bound auth (initially just the WhatsApp opt-in endpoint; future-proofed for abandoned-checkout recovery, mid-checkout push, etc.). The endpoint is rate-limited via the existing `rate_limit_storefront` middleware.
...
- **FR-037**: The pre-send guard MUST verify, in this order: (a) recipient phone is non-empty and E.164-valid, (b) the store has WhatsApp configured (mode + credentials valid), (c) the merchant notification setting for the message type is enabled, (d) the (store, phone) does not have an explicit opt-out row (always enforced except for the STOP-acknowledgement reply itself), (e) **if the template category is marketing**, an active opt-in row exists for the (store, phone) (utility/authentication templates skip this check per FR-011), (f) for non-template messages, the 24h customer-service window is open, (g) **for template-based sends, the referenced `whatsapp_templates` row's `status` is `APPROVED`** (else skip with `template_not_approved`, per FR-029).
- **FR-038**: Guard failures MUST log a skip with a structured reason code (one of: `no_phone`, `invalid_phone`, `no_credentials`, `credentials_invalid`, `merchant_setting_off`, `opt_out`, `no_opt_in`, `window_closed`, `template_not_approved`, `already_sent`) and MUST NOT call the Meta API. The `template_not_approved` reason is emitted whenever a template send is requested for a local `whatsapp_templates` row whose `status` is not `APPROVED` (covers FR-029).
...
- **The storefront-facing opt-in endpoint is no longer freely callable** — it requires a valid checkout-session token (FR-007a). This closes the abuse vector where anyone could write opt-in rows for arbitrary phones. The checkout-session token mechanism is **new infra introduced in this phase** (FR-007b) since NUMU does not have one today — only a `numu_cart_session` HTTP-only cookie that doesn't carry a phone. The token mechanism is Redis-backed (30-min TTL), reuses the existing cart cookie for issuance, and is future-proofed for other anonymous endpoints (abandoned-checkout recovery, mid-checkout push).
