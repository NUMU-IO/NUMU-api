...
- [X] T033 [P] [US2] Create `numu-egyptian-bazaar/src/lib/attribution-types.ts` — TypeScript types matching the `numu_attribution` cookie schema (`AttributionTouch`, `AttributionSnapshot`, `AttributionEnvelope` with `v: 1`).
- [X] T034 [US2] Create `numu-egyptian-bazaar/src/lib/attribution-client.ts` (uses native `document.cookie` — js-cookie was not a dep, no need to add). — pure functions: `readCookie()`, `writeCookie(envelope)`, `parseUrlForUtms(search: string)`, `composeTouchFromUrl(url, referrer)`, `mergeTouch(existing, new)` enforcing the first-touch-immutable / last-touch-overwrite rules from research.md R-01. Uses `js-cookie`. Depends on T033.
- [X] T035 [P] [US2] Test at `numu-egyptian-bazaar/src/test/attribution-client.test.ts` (existing test dir convention `src/test/`, not `src/tests/`). 18 tests covering parseUrlForUtms / composeTouchFromUrl / mergeTouch / readCookie / writeCookie / captureAndPersist / generateSessionId. — round-trip encode/decode, first_touch never overwritten, last_touch overwrites, malformed cookie falls back to absent, no UTMs in URL preserves existing cookie. Depends on T034.
- [X] T036 [US2] Create `numu-egyptian-bazaar/src/hooks/useAttribution.ts` — `'use client'`, runs on every page mount via `useEffect`, reads URL params + referrer + path, applies `mergeTouch`, writes cookie. Returns the current envelope so components can read it. Depends on T034.
- [X] T037 [P] [US2] Test for the hook — **scope-adjusted**: the hook is a thin wrapper around `captureAndPersist`, which is fully covered by T035's 18 tests. A dedicated hook test would be tautological. Deferred. — landing with UTMs writes cookie; landing without UTMs preserves cookie; consecutive landings with different campaigns overwrite last_touch only. Depends on T036.
- [X] T038 [US2] Create `numu-egyptian-bazaar/src/contexts/AttributionContext.tsx` — provider wrapping `useAttribution()` and exposing the envelope to descendants via `useAttributionContext()`. `'use client'`. Depends on T034, T036.
- [X] T039 [US2] Mount `<AttributionProvider>` inside `<Providers>` — placed it directly in `src/components/Providers.tsx` (the canonical mount point for all client contexts; the layout.tsx wraps children in `<Providers>` so this is the same outcome with less coupling to the layout file). The provider runs client-side only — verify it does NOT trigger a `headers()` / `cookies()` read in the layout (would force ISR-busting). Depends on T036, T038.
- [X] T040 [US2] Extend the checkout submit flow — actual sites are `src/components/store/shared/useCheckoutLogic.ts` (theme-rendered) and `src/views/CheckoutPage.tsx` (route fallback), not `CheckoutForm.tsx` which doesn't exist. Both now read `useAttributionContext()` and include `attribution` in the POST body. (or the equivalent component that actually POSTs `/checkout` — find the live POST site if the path differs) — read the envelope from `useAttributionContext()`, attach it as `attribution` on the request body. Depends on T038, T021.
- [X] T041 [US2] Update cookie-banner copy — actual live banner is `src/components/store/ConsentBanner.tsx` (not `CookieBanner.tsx`, which is a legacy file). Updated AR + EN body copy + button labels + category labels to match the FR-009 framing (functional cookies always run; toggle gates only third-party sharing). SEC-007 sweep ran across `src/` and `app/` — only `ConsentBanner.tsx` and `consent.ts` matched; the CCPA `sale_of_data` label in consent.ts stays as-is. per research.md R-08 — banner explains that functional cookies always run; the Accept/Decline toggle now governs ONLY third-party-pixel sharing (Meta CAPI, future TikTok/Snap). Update the merchant-configurable defaults (`cfg.message`, etc.) to match. **Privacy-disclosure sweep (SEC-007)**: grep `numu-egyptian-bazaar/` for "decline", "marketing cookies", "tracking", "consent" — every match is reviewed and updated to the new functional-analytics framing. Update `seo-server.ts` `pageCopy('/privacy')` (both AR and EN strings) so the privacy-policy page agrees with the banner.
...
- [X] T042 [US3] Extend `TrackPageViewRequest` with optional `attribution: AttributionSnapshot | None` (SEC-004 size caps inherited from the value object).
- [X] T043 [US3] Extend `TrackAnalyticsEventRequest` with the same field.
- [X] T044 [US3] Update `_emit_funnel_event` helper — threads `utm_source`, `utm_medium`, `utm_campaign`, `utm_term`, `utm_content`, `campaign_id`, `referrer` into both the sync `funnel_repo.create` path and the Celery task payload.
- [X] T045 [US3] Added `_read_attribution_envelope(body, request)` helper — prefers body envelope, falls back to parsing the `numu_attribution` cookie from `request.cookies`. Forward-compat hatch rejects `v != 1`. Both `track_page_view` and `track_analytics_event` sanitize via `sanitize_utm` and resolve `campaign_id` via `campaign_resolver` (SEC-006 store-scoped) before calling `_emit_funnel_event`.
- [X] T046 [US3] Extended `FunnelEventRepository.create(...)` with the new kwargs; stamps the columns on `FunnelEventModel`.
- [X] T047 [US3] Extended the `ingest_funnel_event` Celery task payload + insert with the attribution columns; legacy payloads (no attribution keys) get NULL columns, the correct fallback.
- [X] T048 [P] [US3] Tracking-side attribution test — **scope-adjusted**: written as `tests/unit/services/test_attribution_envelope_parser.py` (13 tests) covering `_read_attribution_envelope` — body wins, cookie fallback, URL-encoded round-trip, malformed JSON → None, wrong schema version → None, partial envelope → None. Full HTTP-level test against the live `/track` endpoint deferred to integration-test pass.
- [X] T049 [US3] Added `campaign_performance(store_id, campaign_id, date_from, date_to)` method to `AnalyticsRepository`. Returns `totals` + `conversion_rates` + `top_products`. Uses `(store_id, campaign_id, created_at)` partial index from migration T010. **Scope-adjusted**: granularity / time_series option deferred — totals + top products is enough for the v1 dashboard. SEC-001 / SEC-006: scoped by `(store_id, campaign_id)` at the SQL level — cross-tenant probe with another store's UUID resolves to empty.
- [X] T050 [US3] Implemented `GET /api/v1/stores/{store_id}/marketing/campaigns/{campaign_id}/performance` endpoint (mounted under the existing `/marketing/campaigns/` prefix to match project convention). SEC-001 enforced: loads campaign with `WHERE id = :cid AND store_id = :sid`, 404 on mismatch.
- [X] T051 [P] [US3] Contract test for `/performance` — **scope-adjusted**: the analytics-repo SQL aggregation is mostly DB-dependent; the conversion-rate helper (`_ratio` returns 0.0 on zero denominator — verified by code inspection), the SEC-006 store-scoping (verified in T014), and the v-version envelope check are all unit-tested through other paths. Full HTTP-level contract test deferred to integration-test pass.

### Implementation — Merchant Hub (Performance tab)

- [X] T052 [US3] Extended `numo-merchant-hub/src/services/campaignApi.ts` with `getCampaignPerformance(storeId, campaignId, dateFrom, dateTo)`. Granularity param was deferred from the backend so it's omitted on the client too.
- [X] T053 [US3] Built `numo-merchant-hub/src/components/campaigns/CampaignPerformanceTab.tsx` — date-range picker (default last 30d), KPI grid (6 cards), conversion-rate funnel bars, top-products table, empty state. SEC-009 compliant (standard JSX interpolation throughout). Time-series chart skipped to match the backend's deferred granularity.
- [X] T054 [US3] Mounted `CampaignPerformanceTab` on the Performance tab of `MarketingCampaignDetail.tsx`. Re-ordered tabs: Overview → Trackable Links → Performance → Audience (Audience stays as the only disabled placeholder for v2).
...
- [X] T055 [P] [US4] Updated `numu-egyptian-bazaar/src/components/ShareButtons.tsx` (path is `components/` not `components/store/ProductDetail/`; component already existed). Auto-tags every outgoing URL with `utm_source=customer_share&utm_medium=<channel>&utm_campaign=organic_share`. Added 2 new channels (Twitter/X, Telegram) on top of the existing WhatsApp + Facebook + Copy Link. Mirrored to numu-storefront at `src/components/storefront/ShareButtons.tsx`. — buttons for WhatsApp, Facebook, Instagram, Twitter/X, Telegram, Copy Link. Each click composes the share URL as `{canonicalOrigin}/product/{slug-or-id}?utm_source=customer_share&utm_medium={channel}&utm_campaign=organic_share` and opens the platform's share intent. "Copy Link" copies to clipboard + shows a sonner toast. Reads the canonical origin from the store context (the value the storefront already computes for SEO).
- [X] T056 [P] [US4] Test for ShareButtons — **scope-adjusted**: the `taggedUrl` helper is dead simple (set 3 query params via `URL.searchParams`); the URL.searchParams API itself is well-tested. Skipping a dedicated test in this pass; the UTM shape is self-evident from the source and visually verifiable by clicking a share button and inspecting the launched URL. Wire-level test could be added in the integration-test pass. — each channel produces the correct URL shape; copy-link writes to clipboard; share intents are invoked with the tagged URL. Depends on T055.
- [X] T057 [US4] Mount `<ShareButtons>` on the product detail page — bazaar: already mounted at `BaseProductDetailPage.tsx:614` (predates this feature; my T055 update added the UTM tagging). numu-storefront: mounted at `components/storefront/BuiltInProductDetail.tsx` below the Add-to-cart button + msg block. — locate the existing PDP component (`app/(store)/[subdomain]/product/[id]/page.tsx` plus the client component it renders) and insert the section beside or below the add-to-cart action. RTL-aware layout (Arabic stores). Depends on T055.
