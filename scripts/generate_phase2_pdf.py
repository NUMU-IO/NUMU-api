...
class Phase2PDF(FPDF):
    def header(self):
        if self.page_no() == 1:
            ...
            self.cell(0, 12, "NUMU API - Days 31-60: Make It Reliable", new_x="LMARGIN", new_y="NEXT", align="C")
            self.set_font("Helvetica", "", 10)
            self.set_text_color(100, 100, 100)
            self.cell(0, 6, "2-Week Sprint  |  Yousef & Yahia", new_x="LMARGIN", new_y="NEXT", align="C")
    ...
    def footer(self):
        ...
        self.cell(0, 10, f"NUMU API Phase 2 - Days 31-60  |  Page {self.page_no()}/{{nb}}", align="C")
...
def build_pdf():
    ...
    pdf.cell(0, 5, "  Yousef  -  Security, image pipeline & onboarding logic", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 5, "  Yahia   -  Observability, notifications & load testing", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(3)
    ...
    yahia_w1 = [
        ...
        ("7", "Create .env.staging template + staging deployment script + update Makefile with staging commands", ".env.staging, scripts/deploy_staging.sh, Makefile (modify)"),
    ...
    pdf.cell(0, 5, "File scope column shows which directory each PR touches - PRs touching different directories cannot conflict.", new_x="LMARGIN", new_y="NEXT")
    ...
    w1_prs = [
        ...
        ("21", "feat: security headers middleware", "None (independent)", "Yousef", "middleware/"),
        ("22", "feat: Sentry SDK integration + middleware", "None (independent)", "Yahia", "main.py, config/"),
        ("23", "feat: structured logging with structlog (JSON)", "None (independent)", "Yahia", "middleware/logging"),
        ("24", "feat: 2FA entity + TOTP service + backup codes", "None (independent)", "Yousef", "core/entities/, ext_svc/"),
        ("25", "feat: 2FA use cases + API routes (enable/verify/disable)", "PR #24", "Yousef", "use_cases/auth/, routes/"),
        ("26", "feat: enhanced health check (DB, Redis, Sentry status)", "PR #22", "Yahia", "routes/health.py"),
        ("27", "feat: structured logs in key flows (auth, orders, payments)", "PR #23", "Yahia", "use_cases/, webhooks/"),
        ("28", "infra: docker-compose.staging + Nginx + .env.staging + deploy script", "None (independent)", "Yahia", "docker/, scripts/"),
        ("29", "test: RLS isolation + security headers + 2FA + observability", "PR #20, #21, #25, #27", "Yousef", "tests/"),
    ]
    ...
    w2_prs = [
        ...
        ("31", "feat: image optimization pipeline + Celery async task", "PR #30", "Yousef", "ext_svc/image/, tasks/"),
        ("32", "feat: integrate pipeline into product image upload", "PR #31", "Yousef", "use_cases/products/"),
        ("33", "feat: wire WhatsApp + email to order events via Celery tasks", "None (independent)", "Yahia", "use_cases/orders/, tasks/"),
        ("34", "feat: notification preferences (opt-in/out) + migration", "None (independent)", "Yahia", "entities/customer, alembic/"),
        ("35", "feat: onboarding entity + wizard use case + progress tracking", "None (independent)", "Yousef", "core/, use_cases/stores/"),
        ("36", "feat: onboarding routes + auto-trigger step completion", "PR #35", "Yousef", "routes/stores/, schemas/"),
        ("37", "feat: welcome + onboarding email sequence (Celery scheduled)", "PR #34", "Yahia", "tasks/, resend/templates/"),
        ("38", "feat: Locust load test suite (smoke/load/stress profiles)", "None (independent)", "Yahia", "tests/load/"),
        ("39", "test: image pipeline + onboarding + notifications + load smoke", "PR #32, #36, #33, #38", "Both", "tests/"),
    ]
    ...
    pdf.cell(0, 5, "Shared Files (merge sequentially - never in parallel):", new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(0, 0, 0)
