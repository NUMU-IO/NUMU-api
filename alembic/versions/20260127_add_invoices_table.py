...
down_revision: str | None = "2d2b2176a338"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "invoice_type",
            postgresql.ENUM(
                "I", "C", "D", name="invoicetype", schema="public", create_type=False
            ),
            nullable=False,
            server_default="I",
        ),
        ...
        sa.ForeignKeyConstraint(
            ["order_id"], ["public.orders.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(
            ["customer_id"], ["public.customers.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(
            ["related_invoice_id"], ["public.invoices.id"], ondelete="SET NULL"
        ),
        schema="public",
    )
    ...
    op.create_index(
        "ix_invoices_customer_id", "invoices", ["customer_id"], schema="public"
    )
    op.create_index(
    ...
    op.create_index(
        "ix_invoices_date_issued", "invoices", ["date_issued"], schema="public"
    )
